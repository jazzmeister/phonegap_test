var imageLib = {
    ...
    downloadImages: function(urls, onCompletion, onFailure /* :function */, filenamePrefix /* :string */) {
        if(urls.length == 0) {
        ...
        var filename = filenamePrefix + counter + '.png';
        ...
        appLib.downloadFile(url,
                            ...
                            function(url, file) {
                                ...
                                imageLib.downloadImages(otherUrls, onCompletion, onFailure, filenamePrefix);
                            },
                            function(error) {
                                appLib.log('image download failed');
								appLib.log(error);
                            
                                if(_.isUndefined(onFailure)) {
                                    //## Ignore failed download and continue 
                                    imageLib.downloadImages(otherUrls, onCompletion, onFailure, filenamePrefix);
                                } else {
                                    onFailure(error);
                                }
